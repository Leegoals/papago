...
// 그 콜백함수는 2개의 인수(argument)를 받음, 1. request(줄여서 req), response(res)
// HttpServletRequest, HttpServletResponse정도 개념
app.get('/', (req, res) => {
    // res.send('응답 완료!'); // 동작 확인용
    // root url, 즉 메인 페이지로 접속했을 때, 우리가 만든 Node서버는 papago의 메인 화면인 public/index.html을 응답해줘야 함
...
// 서버가 실행되었을 때 몇 번 포트에서 실행시킬 것인지(3000번 포트)
app.listen(3000, () => console.log('http://127.0.0.1:3000/ app listening on port 3000'));

// Node.js기반의 js파일 실행 시에는 node로 시작하는 명령어에 파일명까지 작성하면됨
